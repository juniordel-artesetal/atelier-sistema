...
const REGIMES = [
  {
    key: 'MEI',
    label: 'MEI',
    desc: 'Microempreendedor Individual',
    aliquota: 0,
  ...
  },
  {
    ...
    desc: 'Microempresa ou Empresa de Pequeno Porte',
    aliquota: 6,
  ...
  },
  {
    ...
    desc: 'Tributação sobre lucro estimado',
    aliquota: 11.33,
  ...
  },
  {
    key: 'LUCRO_REAL',
    label: 'Lucro Real',
    desc: 'Tributação sobre lucro efetivo',
    aliquota: 15,
...
export default function ConfigTributosPage() {
  ...
  const [form, setForm] = useState({
    regime: 'MEI', aliquotaPadrao: '0', observacoes: ''
  })
      ...
      .then(data => {
        if (data) {
          ...
          setForm({
            ...
            aliquotaPadrao: String(data.aliquotaPadrao || 0),
  ...
  async function handleSave() {
    ...
    try {
      const res = await fetch('/api/precificacao/config-tributos', {
        ...
        body: JSON.stringify({
          regime: form.regime,
          aliquotaPadrao: Number(form.aliquotaPadrao),
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Define o regime tributário da empresa e a alíquota padrão usada nos cálculos de precificação.
        </p>
      </div>
        ...
        <div className="max-w-2xl space-y-6">
            ...
            <div className="grid grid-cols-2 gap-3">
              {REGIMES.map(r => (
                <button key={r.key} onClick={() => selectRegime(r.key)}
                  ...
                  <p className={`font-semibold text-sm ${form.regime === r.key ? 'text-purple-700' : 'text-gray-800'}`}>
                    {r.label}
                  </p>
                  <p className="text-xs text-gray-500 mt-0.5">{r.desc}</p>
                  <p className={`text-xs font-bold mt-1.5 ${form.regime === r.key ? 'text-purple-600' : 'text-gray-400'}`}>
                    Sugestão: {r.aliquota}%
                  </p>
                </button>
              ))}
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <div className="flex items-center gap-3">
              <input
                ...
                value={form.aliquotaPadrao}
                onChange={e => setForm(p => ({ ...p, aliquotaPadrao: e.target.value }))}
                className={inputClass + ' text-2xl font-bold text-purple-700 max-w-40'}
              />
              <span className="text-2xl text-gray-400 font-bold">%</span>
            </div>
            <p className="text-xs text-gray-400 mt-2">
              Exemplo: para um produto de R$ 100,00 com {aliquota}% de imposto → R$ {(100 * aliquota / 100).toFixed(2)} de imposto
            </p>
                ...
                {[30, 60, 100].map(preco => (
                  <div key={preco} className="bg-gray-50 rounded-lg px-3 py-2 text-center">
                    ...
                    <p className="text-sm font-bold text-red-500">
                      - R$ {(preco * aliquota / 100).toFixed(2)}
                    </p>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-6">
            ...
            <textarea
              ...
              placeholder="Ex: Produtos com NCM 6217.10.00 têm alíquota de IPI reduzida. NCM 4819.20.00 isento de IPI..."
            />
            <p className="text-xs text-gray-400 mt-1">
              Use este campo para registrar exceções ou observações do seu contador.
            </p>
          </div>
          ...
          <div className="flex items-center gap-4">
            <button onClick={handleSave} disabled={saving}
              className="bg-purple-600 hover:bg-purple-700 text-white font-semibold px-6 py-2.5 rounded-lg disabled:opacity-50">
              {saving ? 'Salvando...' : 'Salvar configuração'}
            </button>
            {saved && (
              <span className="text-green-600 text-sm font-medium">✅ Configuração salva!</span>
            )}
