...
function getTaxa(canal: string, sub: string, preco: number): { taxa: number; fixo: number; label: string } {
  if (canal === 'shopee') {
    if (preco < 80)  return { taxa: 0.20, fixo: 4.00,  label: 'Shopee até R$79,99 · 20%+R$4' }
    if (preco < 100) return { taxa: 0.14, fixo: 16.00, label: 'Shopee R$80–99 · 14%+R$16' }
    if (preco < 200) return { taxa: 0.14, fixo: 20.00, label: 'Shopee R$100–199 · 14%+R$20' }
    return             { taxa: 0.14, fixo: 26.00, label: 'Shopee +R$200 · 14%+R$26' }
  }
            ...
            <div className="sticky top-0 bg-white rounded-t-2xl px-6 pt-5 pb-4 border-b border-gray-100 z-10">
              ...
              <div className="flex items-center justify-between bg-gray-50 rounded-xl px-4 py-3">
                ...
                <label className="relative inline-flex items-center cursor-pointer">
                  ...
                  <div className="w-11 h-6 bg-gray-200 peer-focus:ring-2 peer-focus:ring-purple-400 rounded-full peer peer-checked:after:translate-x-full after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-5 after:w-5 after:transition-all peer-checked:bg-purple-600" />
                </label>
              </div>
              ...
              {conf.isKit && (
                <div className="mt-3 flex items-center gap-3">
                  ...
                  <input type="number" min="2" step="1" value={conf.qtdKit}
                    ...
                    className={inputClass + ' max-w-28 font-bold text-purple-700'} />
            ...
            <div className="p-6 space-y-5">
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button onClick={addMat} className="text-xs text-purple-600 hover:underline font-medium">
                    + Adicionar material
                  </button>
                </div>
                    ...
                    <div key={i} className="mb-3 p-3 bg-gray-50 rounded-xl border border-gray-100">
                      ...
                      {novoMatIdx === i && (
                        <div className="mt-3 p-3 bg-purple-50 rounded-xl border border-purple-100 space-y-2">
                          <p className="text-xs font-semibold text-purple-700">Cadastrar novo material</p>
                  ...
                  <div className="flex gap-2 mb-2">
                    {(['local', 'freelancer'] as const).map(t => (
                      <button key={t} onClick={() => setConf(p => ({ ...p, tipoMaoObra: t, custoMaoObra: t === 'local' ? '0' : p.custoMaoObra }))}
                        className={`flex-1 py-1.5 rounded-lg text-xs font-semibold border-2 transition-all ${conf.tipoMaoObra === t ? 'bg-purple-50 border-purple-400 text-purple-700' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                  ...
                  <div className="flex gap-2 mb-2">
                    {(['local', 'freelancer'] as const).map(t => (
                      <button key={t} onClick={() => setConf(p => ({ ...p, tipoArte: t, custoArte: t === 'local' ? '0' : p.custoArte }))}
                        className={`flex-1 py-1.5 rounded-lg text-xs font-semibold border-2 transition-all ${conf.tipoArte === t ? 'bg-purple-50 border-purple-400 text-purple-700' : 'border-gray-200 text-gray-500 hover:bg-gray-50'}`}>
                        {t === 'local' ? '🏠 Local (gratuito)' : '👤 Freelancer'}
                      </button>
                    ))}
                  </div>
                  ...
                  <div>
                    <div className="flex items-center justify-between mb-1">
                      ...
                      <button onClick={() => setConf(p => ({ ...p, custosAdicionais: [...p.custosAdicionais, { descricao: '', valor: '' }] }))}
                        className="text-xs text-purple-600 hover:underline">+ Adicionar</button>
                    </div>
                    {conf.custosAdicionais.map((c, i) => (
                      <div key={i} className="flex gap-1 mb-1">
                        <input value={c.descricao}
                          ...
                          className={inputClass} placeholder="Descrição" />
                        <input type="number" step="0.01" value={c.valor}
                          ...
                          className={inputClass + ' w-24 flex-shrink-0'} placeholder="R$" />
              ...
              {custoLote > 0 && (
                <div>
                  <div className="flex items-center justify-between mb-2">
                    ...
                    <span className="text-xs bg-purple-50 text-purple-700 px-2 py-0.5 rounded-full">{canalSel.label}</span>
                  </div>
                  <div className="grid grid-cols-3 gap-2 mb-3">
                    {[
                      ...
                      <button key={label}
                        ...
                        className={`p-3 rounded-xl border-2 text-left transition-all hover:opacity-80 active:scale-95 ${cor} ${conf.precoVenda === val?.toFixed(2) ? 'ring-2 ring-purple-400' : ''}`}>
                  ...
                  <div>
                    <label className="block text-xs font-medium text-gray-500 mb-1">
                      Preço de venda <span className="text-purple-400 ml-1">← selecione uma sugestão ou digite</span>
                    </label>
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      ...
                      {prod.categoria && <span className="bg-purple-50 text-purple-600 px-2 py-0.5 rounded-full mr-2">{prod.categoria}</span>}
                          ...
                          return (
                            <tr key={c.id} className="border-t border-gray-50 hover:bg-gray-50/40">
                              <td className="px-4 py-2.5">
                                <span className={`text-xs px-2 py-0.5 rounded-full font-medium ${c.isKit ? 'bg-orange-50 text-orange-700' : 'bg-purple-50 text-purple-700'}`}>
