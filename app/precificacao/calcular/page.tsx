            ...
            <div className="flex gap-2 mb-4">
              {[{ k: 'produto', l: 'Produto cadastrado' }, { k: 'manual', l: 'Inserir manualmente' }].map(m => (
                <button key={m.k} onClick={() => setModo(m.k as any)}
                  className={`flex-1 text-sm py-2 rounded-lg font-medium transition-colors ${modo === m.k ? 'bg-purple-600 text-white' : 'border border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
                  {m.l}
                </button>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-2">
                {CANAIS.map(c => (
                  <button key={c.key} onClick={() => { setCanal(c.key); setResultado(null); setCalcTodos(false) }}
                    className={`flex items-center gap-2 px-3 py-2 rounded-lg text-sm font-medium border transition-all ${canal === c.key ? 'bg-purple-600 text-white border-purple-600' : 'border-gray-200 text-gray-600 hover:bg-gray-50'}`}>
            ...
            {(canal === 'shopee' || canal === 'tiktokshop') && (
              <label className="flex items-center gap-2 text-sm cursor-pointer">
                <input type="checkbox" checked={freteGratis} onChange={e => setFreteGratis(e.target.checked)} className="accent-purple-600" />
            ...
            <div className="flex gap-2">
              ...
              <button onClick={calcularTodos} disabled={calculando}
                className="flex-1 border border-purple-300 text-purple-700 hover:bg-purple-50 font-medium py-2.5 rounded-lg disabled:opacity-50 text-sm">
