'use client'
// app/precificacao/embalagens/page.tsx
            ...
            <tbody>
              {filtered.map(emb => (
                <tr key={emb.id} className="border-t border-gray-50 hover:bg-gray-50/50">
                  ...
                  <td className="px-4 py-3 text-right font-bold text-purple-700">{fmtR2(emb.custoTotal)}</td>
                  <td className="px-4 py-3 text-center">
                    <div className="flex items-center justify-center gap-3">
                      <button type="button" onClick={() => openModal(emb)}
                        className="text-xs text-blue-500 hover:underline">Editar</button>
                      {confirmDelId === emb.id ? (
                        <div className="flex items-center gap-1">
                          <span className="text-xs text-red-600 font-medium">Confirmar?</span>
                          <button type="button" onClick={() => handleDelete(emb.id)}
                          ...
                          <button type="button" onClick={() => setConfirmDelId(null)}
                            className="text-xs border border-gray-300 text-gray-500 px-2 py-0.5 rounded">Não</button>
                        </div>
                      ) : (
                        <button type="button" onClick={() => setConfirmDelId(emb.id)}
          ...
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-2xl mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="px-6 py-5 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input value={nome} onChange={e => setNome(e.target.value)}
                    placeholder="Ex: Embalagem Cofrinho Kit 10" className={inputClass} />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">Descrição</label>
                  <input value={descricao} onChange={e => setDescricao(e.target.value)}
                    placeholder="Ex: Saco + Transparente + Fita" className={inputClass} />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center justify-between mb-2">
                  ...
                  <button type="button" onClick={() => setItens(prev => [...prev, { ...ITEM_VAZIO }])}
                    className="text-xs text-purple-600 hover:underline">+ Adicionar material</button>
                </div>
              ...
              <div className="bg-purple-50 border border-purple-200 rounded-xl px-4 py-3 flex items-center justify-between">
                <div>
                  <p className="text-xs text-purple-600 font-medium">Custo total da embalagem</p>
                  <p className="text-xs text-gray-400">Custo fixo por venda (independe da qtd do kit)</p>
                </div>
                <p className="text-xl font-bold text-purple-700">{fmtR2(custoCalc)}</p>
              </div>
            </div>
