...
// ── Faixas de recomendação por faturamento anual ──────────────────────────────
const FAIXAS_RECOMENDACAO = [
  {
    faixaMin:0, faixaMax:81000, label:'Até R$ 81 mil/ano',sublabel:'até R$ 6.750/mês',
    recomendado:'MEI', cor:'text-yellow-700', bg:'bg-yellow-50', border:'border-yellow-300',
    aliquotaEfetiva:'~0,09%',
    razao:'MEI é o mais econômico nesta faixa. Taxa fixa de R$75,90/mês independente do faturamento. Economia de até R$700/mês vs Simples.',
    atencao:'Limite estrito de R$81.000/ano. Ultrapassar exige migração imediata para Simples Nacional.',
    alternativa: null,
  },
  {
    faixaMin:81000, faixaMax:360000, label:'R$ 81 mil – R$ 360 mil/ano', sublabel:'R$ 6.750 – R$ 30.000/mês',
    recomendado:'Simples Nacional', cor:'text-green-700', bg:'bg-green-50', border:'border-green-300',
    aliquotaEfetiva:'4,5% – 5,5%',
    razao:'Simples Nacional Faixa 1/2 é ideal. Alíquota efetiva entre 4,5% e 5,5% — muito abaixo do Lucro Presumido (~11%). Um DAS unificado substitui vários impostos.',
    atencao:'Evite Lucro Presumido nesta faixa — carga efetiva seria ~2x maior.',
    alternativa:null,
  },
  {
    faixaMin:360000, faixaMax:720000, label:'R$ 360 mil – R$ 720 mil/ano', sublabel:'R$ 30.000 – R$ 60.000/mês',
    recomendado:'Simples Nacional', cor:'text-green-700', bg:'bg-green-50', border:'border-green-300',
    aliquotaEfetiva:'6,5% – 8%',
    razao:'Simples ainda vantajoso nas Faixas 2/3. Alíquota efetiva cresce com o faturamento, mas permanece abaixo do Lucro Presumido para a maioria dos negócios.',
    atencao:'Compare com Lucro Presumido se tiver folha de pagamento reduzida — pode haver equiparação de custos.',
    alternativa:'Lucro Presumido (se folha salarial < 20% receita)',
  },
  {
    faixaMin:720000, faixaMax:1800000, label:'R$ 720 mil – R$ 1,8 mi/ano', sublabel:'R$ 60.000 – R$ 150.000/mês',
    recomendado:'Comparar', cor:'text-orange-700', bg:'bg-orange-50', border:'border-orange-300',
    aliquotaEfetiva:'8% – 11%',
    razao:'Zona de transição. Simples Faixas 3/4 começa a se aproximar do Lucro Presumido. A melhor opção depende da estrutura de custos, folha e margens reais.',
    atencao:'Recomendado contratar contador para simulação personalizada antes de decidir.',
    alternativa:'Lucro Presumido pode ser equivalente ou melhor',
  },
  {
    faixaMin:1800000, faixaMax:4800000, label:'R$ 1,8 mi – R$ 4,8 mi/ano', sublabel:'R$ 150.000 – R$ 400.000/mês',
    recomendado:'Lucro Presumido', cor:'text-blue-700', bg:'bg-blue-50', border:'border-blue-300',
    aliquotaEfetiva:'11% – 14%',
    razao:'Lucro Presumido tende a ser mais vantajoso nesta faixa. Simples Faixas 5/6 tem alíquota de até 30%, tornando o Presumido mais econômico para a maioria dos casos.',
    atencao:'Se a margem líquida for baixa (<15%), o Lucro Real pode ser mais interessante.',
    alternativa:'Lucro Real (se margem líquida < 15%)',
  },
  {
    faixaMin:4800000, faixaMax:Infinity, label:'Acima de R$ 4,8 mi/ano', sublabel:'acima de R$ 400.000/mês',
    recomendado:'Lucro Real', cor:'text-purple-700', bg:'bg-purple-50', border:'border-purple-300',
    aliquotaEfetiva:'Variável',
    razao:'Lucro Real é obrigatório acima de R$ 78 mi/ano, mas é frequentemente a melhor opção após R$ 4,8 mi. Permite compensar prejuízos, tomar créditos de PIS/COFINS e ter base de cálculo justa.',
    atencao:'Exige contabilidade detalhada e competente. Não recomendado sem estrutura contábil profissional.',
    alternativa:'Lucro Presumido (se operação simples e margem alta)',
  },
]
...
export default function OraculoPage() {
  ...
  // Faixa de recomendação baseada no RBT12
  ...
  async function buscarNCM() {
    ...
    try {
      const res = await fetch('/api/precificacao/ncm', {method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({produto:ncmBusca})})
  ...
  const tabCores:Record<string,string> = {
    mei:'bg-yellow-600 text-white', simples:'bg-green-600 text-white',
    presumido:'bg-blue-600 text-white', real:'bg-purple-600 text-white',
  }
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[300px_1fr] gap-5">
        ...
        <div className="space-y-4">
              ...
              <div key={g.grupo}>
                ...
                {CANAIS.filter(c => g.filter(c.key)).map(c => (
                  <div key={c.key} onClick={() => setCanalKey(c.key)}
                    className={`flex items-center justify-between p-2.5 rounded-xl cursor-pointer border-2 mb-1.5 transition-all ${canalKey===c.key?'border-purple-400 bg-purple-50':'border-gray-100 hover:border-gray-200'}`}>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            ...
            <div className="mt-2 bg-blue-50 rounded-lg px-3 py-2 text-xs text-blue-700 font-medium">
              ICMS {estadoUF}: <strong>{(icmsAliq*100).toFixed(1)}%</strong> = {fmt(icmsValor)}
            </div>
          </div>
        </div>
        ...
        <div className="space-y-4">
          ...
          {/* Comparativo */}
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <p className="text-xs font-bold text-purple-600 uppercase tracking-widest mb-4">📊 Comparativo — {canal.label} incluso</p>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
              {[
                {id:'mei',      label:'MEI',             sub:'Taxa fixa',          total:totalMEI, imposto:MEI_DAS_MENSAL, cor:'text-yellow-700', bg:'bg-yellow-50', border:'border-yellow-300'},
                {id:'simples',  label:'Simples Nacional', sub:'Anexo II',           total:totalS,   imposto:dasTotal,       cor:'text-green-700',  bg:'bg-green-50',  border:'border-green-300' },
                ...
                <div key={r.id} onClick={() => setTab(r.id as any)}
                  className={`relative p-3 rounded-xl border-2 cursor-pointer transition-all ${tab===r.id?r.border+' '+r.bg:'border-gray-100 hover:border-gray-200'} ${r.total===maisBarato?'ring-2 ring-green-400':''}`}>
                  {r.total===maisBarato && <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-green-500 text-white text-xs font-bold px-2 py-0.5 rounded-full whitespace-nowrap">+ econômico</span>}
                  ...
                  <div className="mt-1.5 pt-1.5 border-t border-gray-100 space-y-0.5">
                    <div className="flex justify-between text-xs"><span className="text-gray-400">Impostos</span><span className="font-mono text-gray-600">{fmt(r.imposto)}</span></div>
                    <div className="flex justify-between text-xs"><span className="text-gray-400">Carga</span><span className={`font-mono font-bold ${r.cor}`}>{pct(receita>0?r.imposto/receita:0)}</span></div>
                  </div>
                </div>
              ))}
            </div>
          ...
          <div className="bg-white rounded-xl border border-gray-100 p-5">
            <div className="flex gap-2 flex-wrap mb-5">
              {[{id:'mei',label:'MEI'},{id:'simples',label:'Simples'},{id:'presumido',label:'Lucro Presumido'},{id:'real',label:'Lucro Real'}].map(t => (
            ...
            {tab==='mei' && (
              <div>
                <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3 mb-4 text-xs space-y-1.5">
                  <p className="font-bold text-yellow-800">MEI — Taxa fixa mensal</p>
                  <p className="text-gray-600">DAS-MEI 2025 (Comércio/Indústria): <strong className="text-yellow-700">{fmt(MEI_DAS_MENSAL)}/mês</strong></p>
                  <p className="text-gray-500">Inclui: INSS (R$71,60) + ICMS (R$1,00) + ISS (R$5,00) · Limite: R$81.000/ano</p>
                </div>
                <TribLine label="DAS-MEI (fixo mensal)" valor={MEI_DAS_MENSAL} total={MEI_DAS_MENSAL} color="#d97706"/>
                <div className="mt-3 bg-green-50 border border-green-200 rounded-lg p-3 text-xs text-green-700">
                  <p className="font-bold mb-1">✅ Economia vs Simples</p>
                  <p>Com {fmt(receita)}/mês, MEI economiza <strong>{fmt(Math.max(dasTotal-MEI_DAS_MENSAL,0))}/mês</strong> vs Simples Nacional.</p>
                </div>
            ...
            {tab==='simples' && (
              <div>
                <div className="flex gap-2 mb-4 flex-wrap">
                  <span className="text-xs bg-green-50 text-green-700 border border-green-200 px-2 py-0.5 rounded-full font-semibold">Anexo II — Faixa {simples.idx+1}/6 · {simples.faixa.label}</span>
                </div>
            ...
            {tab==='presumido' && (
              <div>
                <div className="bg-gray-50 rounded-lg p-3 mb-4 text-xs space-y-1">
                  ...
                  <p className="text-gray-500">CSLL: 12% presunção × 9% = <span className="text-purple-600 font-bold">1,08%</span> da receita</p>
                </div>
      ...
      {/* ══ RECOMENDAÇÃO TRIBUTÁRIA POR FATURAMENTO ═══════════════════════════ */}
      <div className="bg-white rounded-xl border border-gray-100 p-6">
        <div className="flex items-center justify-between mb-4">
          <div>
            <p className="text-xs font-bold text-purple-600 uppercase tracking-widest">🎯 Melhor Regime por Volume de Faturamento</p>
            <p className="text-sm text-gray-500 mt-1">Estudo comparativo — orientação geral para planejamento tributário</p>
          </div>
          {/* Destaque da faixa atual */}
            ...
            return (
              <div key={i} className={`rounded-xl border-2 p-4 transition-all ${ativa ? f.border+' '+f.bg+' shadow-sm' : 'border-gray-100 bg-gray-50/30'}`}>
                <div className="flex flex-col md:flex-row md:items-start gap-3">
                  {/* Faixa e recomendação */}
                  <div className="md:w-48 flex-shrink-0">
                    <div className="flex items-center gap-2">
                      {ativa && <span className="w-2 h-2 rounded-full bg-current flex-shrink-0" style={{backgroundColor: ativa ? '#7c3aed' : '#d1d5db'}}/>}
                  ...
                  <div className="flex-1 space-y-2">
                    ...
                    {f.atencao && (
                      <p className={`text-xs ${ativa ? 'text-orange-600' : 'text-gray-400'}`}>
                        ⚠️ {f.atencao}
                      </p>
                    )}
                    {f.alternativa && (
                      <p className={`text-xs ${ativa ? 'text-blue-600' : 'text-gray-400'}`}>
        ...
        <p className="text-xs text-gray-400 text-center mt-4 italic">
          Recomendações baseadas em alíquotas vigentes 2025 · Cada caso é único — consulte um contador para decisão definitiva
        </p>
      </div>
          ...
          {cnpjData && (
            <div className="mt-3 space-y-2 text-xs">
              <div className="bg-gray-50 rounded-lg p-3 space-y-1.5">
                ...
                <p className="text-gray-500">Natureza: <span className="font-medium text-gray-700">{cnpjData.natureza_juridica}</span></p>
              ...
              {(cnpjData.cnae_fiscal_descricao || cnpjData.cnaes_secundarios?.length > 0) && (
              ...
              <p className="text-gray-400 italic">⚠️ Inscrição estadual/municipal: consultar SEFAZ do estado.</p>
            </div>
          )}
        </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 p-5">
          ...
          <div className="flex gap-2 mb-2">
            <input value={ncmBusca} onChange={e => setNcmBusca(e.target.value)}
              placeholder="Ex: laço cetim, cofrinho papelão, amigurumi..."
          ...
          {ncmLoading && <div className="bg-purple-50 rounded-lg p-3 text-xs text-purple-600 animate-pulse">🔍 Consultando base tributária...</div>}
          ...
          {ncmId && (
            <p className="text-xs text-green-700 bg-green-50 rounded-lg px-3 py-2 mt-3">
              ✅ NCM {ncmId} selecionado — usado no cálculo de ICMS acima
            </p>
          )}
        </div>
      </div>
