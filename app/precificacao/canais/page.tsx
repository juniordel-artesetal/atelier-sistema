  ...
  const load = useCallback(async () => {
    ...
    const prods = (Array.isArray(data) ? data : []).map((p: any) => ({
      ...
      configs: (p.variacoes || []).map((v: any) => ({
        ...
        canal: v.canal || 'shopee', subOpcao: v.subOpcao || 'classico',
      }))
    }))
    setProdutos(prods)
    setLoading(false)
    ...
    <div>
      <div className="mb-6">
        ...
        <p className="text-gray-500 text-sm mt-1">
          Compare o preço ideal de cada produto em todos os canais com cenários de margem baixa, saudável e alta.
        </p>
      </div>
      ...
      <div className="flex flex-wrap gap-3 mb-5">
        ...
        />
        <select
          value={prodSel} onChange={e => setProdSel(e.target.value)}
        ...
        <button
          onClick={() => setExpanded(new Set(produtosFiltrados.map(p => p.id)))}
          className="text-xs text-purple-600 border border-purple-200 px-3 py-2 rounded-lg hover:bg-purple-50"
        >
          Expandir todos
        </button>
        <button
      ...
      <div className="flex gap-4 mb-4 text-xs text-gray-500">
        ...
        <span className="flex items-center gap-1.5"><span className="w-3 h-3 rounded-full bg-purple-300 inline-block" />Preço atual definido</span>
      </div>
        ...
        <div className="space-y-4">
          {produtosFiltrados.map(produto => (
            <div key={produto.id} className="bg-white rounded-xl border border-gray-100">
              ...
              {/* Header produto */}
              <div
                className="flex items-center justify-between px-5 py-4 cursor-pointer hover:bg-gray-50/50 rounded-xl"
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      ...
                      {produto.categoria && <span className="bg-purple-50 text-purple-600 px-2 py-0.5 rounded-full mr-2">{produto.categoria}</span>}
                ...
                return (
                  <div key={config.id} className="border-t border-gray-50 px-5 pb-5">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-3">
                      {CANAIS.map(canal => {
                        // Para ML e Elo7 mostra ambas as sub-opções
                        ...
                        return subOpcoes.map(sub => {
                          ...
                          return (
                            <div key={`${canal.key}-${sub.key}`}
                              ...
                              <div className="flex items-center justify-between mb-2">
                                ...
                                {margemAtual !== null && (
                                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${
                                    margemAtual >= 35 ? 'bg-green-50 text-green-700'
                                    : margemAtual >= 20 ? 'bg-yellow-50 text-yellow-700'
                              ...
                              {/* Shopee: mostra faixa aplicada */}
                              {canal.key === 'shopee' && (
                                <p className="text-xs text-purple-500 mt-1.5 text-center">
                                  📊 Faixa calculada pelo preço de referência
                                </p>
                              )}
                            </div>
                          )
