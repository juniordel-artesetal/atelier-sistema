    ...
    <div className="p-6 space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
        {MESES.map((nomeMes, i) => {
          ...
          return (
            <div key={mes} className={`bg-white rounded-xl border shadow-sm overflow-hidden ${isHoje ? 'border-purple-300 ring-2 ring-purple-200' : 'border-gray-100'}`}>
              <div className={`flex items-center justify-between px-4 py-3 ${isHoje ? 'bg-purple-600 text-white' : 'bg-gray-50 border-b border-gray-100'}`}>
                <div className="flex items-center gap-2">
                  <Target className={`w-4 h-4 ${isHoje ? 'text-purple-200' : 'text-gray-400'}`} />
                ...
                <button onClick={() => isEdit ? setEditMes(null) : openEdit(mes)}
                  className={`text-xs px-2 py-1 rounded ${isHoje ? 'bg-white/20 text-white hover:bg-white/30' : 'bg-purple-50 text-purple-700 hover:bg-purple-100'}`}>
                ...
                {isEdit ? (
                  <div className="space-y-3">
                    ...
                    <button onClick={() => handleSave(mes)} disabled={saving === mes}
                      className="w-full flex items-center justify-center gap-2 py-2 bg-purple-600 text-white rounded-lg text-sm font-medium hover:bg-purple-700 disabled:opacity-50">
