                ...
                return (
                  <React.Fragment key={d.dia}>
                    <tr
                      ...
                      className={`border-t border-gray-50 transition-colors ${isHoje ? 'bg-purple-50' : temMov ? 'hover:bg-gray-50 cursor-pointer' : ''}`}>
                      <td className={`text-center px-4 py-2.5 font-bold text-sm ${isHoje ? 'text-purple-700' : 'text-gray-400'}`}>
                        {String(d.dia).padStart(2, '0')}
                        {isHoje && <span className="block text-[10px] text-purple-500 font-normal">hoje</span>}
                      </td>
