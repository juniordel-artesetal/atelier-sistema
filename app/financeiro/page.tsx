...
export default function DashboardFinanceiro() {
  ...
  return (
    <div className="p-6 space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        <div className="flex items-center gap-2">
          ...
          <button onClick={fetchData}
            className="flex items-center gap-1.5 px-3 py-1.5 text-sm text-purple-700 bg-purple-50 rounded-lg hover:bg-purple-100 ml-2">
      ...
      {loading && (
        <div className="flex items-center justify-center py-8 text-purple-600 gap-2">
        ...
        <>
            ...
            <div className="bg-white rounded-xl border border-gray-100 p-5 shadow-sm">
              ...
              <ResponsiveContainer width="100%" height={240}>
                <LineChart data={data.chart}>
                  ...
                  <Tooltip content={<CustomTooltip />} />
                  <Legend wrapperStyle={{ fontSize: 11 }} />
                  <Line dataKey="resultado" name="Resultado" stroke="#7c3aed" strokeWidth={2.5}
          ...
          <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
            <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between">
              ...
              <a href="/financeiro/fluxo" className="text-xs text-purple-600 hover:underline">Ver por dia →</a>
            </div>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-gray-800 text-white">
                    ...
                    {MESES_ABR.map(m => (
                      <th key={m} className={`text-right px-3 py-2.5 text-xs font-semibold ${m === MESES_ABR[mes - 1] ? 'bg-purple-700' : ''}`}>{m}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {[
                    ...
                    <tr key={key} className="border-t border-gray-100 hover:bg-gray-50/50">
                      ...
                      {data.fluxo.map(f => {
                        ...
                        return (
                          <td key={f.mes} className={`text-right px-3 py-2 text-xs ${cls} ${f.mes === mes ? 'bg-purple-50/50' : ''} ${v < 0 ? '!text-red-600' : ''}`}>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {[
              ...
              <a key={l.href} href={l.href}
                className="bg-white rounded-xl border border-gray-100 p-4 hover:border-purple-300 hover:shadow-sm transition-all group">
                <p className="font-semibold text-gray-700 group-hover:text-purple-700 text-sm">{l.label}</p>
