...
export async function GET(req: Request) {
  ...
  const realizados: any[] = await prisma.$queryRaw`
    ...
    FROM "FinLancamento"
    WHERE "workspaceId"='ws_atelier' AND EXTRACT(YEAR FROM data)=${ano} AND EXTRACT(MONTH FROM data)=${mes} AND status='PAGO'
  ...
  const pendentes: any[] = await prisma.$queryRaw`
    ...
    FROM "FinLancamento"
    WHERE "workspaceId"='ws_atelier' AND EXTRACT(YEAR FROM data)=${ano} AND EXTRACT(MONTH FROM data)=${mes} AND status='PENDENTE'
    GROUP BY dia ORDER BY dia
  `
  ...
  const lancamentos: any[] = await prisma.$queryRaw`
    ...
    WHERE l."workspaceId"='ws_atelier' AND EXTRACT(YEAR FROM l.data)=${ano} AND EXTRACT(MONTH FROM l.data)=${mes}
