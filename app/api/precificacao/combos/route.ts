...
export async function GET() {
  try {
    ...
    const combos = await prisma.$queryRaw`
      ...
      WHERE c."workspaceId" = 'ws_atelier' AND c."ativo" = true
      GROUP BY c."id"
...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.$executeRaw`
      ...
      VALUES (
        ${id}, 'ws_atelier', ${nome}, ${descricao||null},
        ${canal||'shopee'}, ${subOpcao||'classico'},
        ${precoNormal?Number(precoNormal):null},
        ${descontoPct?Number(descontoPct):null},
        ${precoCombo?Number(precoCombo):null}
      )
    `
