...
export async function GET() {
  try {
      ...
      SELECT e.*,
        ...
        COALESCE(
          json_agg(
            json_build_object(
              'id',          ei."id",
              'materialId',  ei."materialId",
              'nomeMaterial',ei."nomeMaterial",
              'qtdUsada',    ei."qtdUsada"::float,
              'custoUnit',   ei."custoUnit"::float,
              'rendimento',  ei."rendimento"::float
            ) ORDER BY ei."id"
          ) FILTER (WHERE ei."id" IS NOT NULL), '[]'
        ) AS itens
      FROM "PrecEmbalagem" e
      LEFT JOIN "PrecEmbalagemItem" ei ON ei."embalagemId" = e."id"
      WHERE e."workspaceId" = 'ws_atelier' AND e."ativo" = true
      GROUP BY e."id"
      ORDER BY e."nome" ASC
    `
    ...
    console.error(error)
    return NextResponse.json({ error: 'Erro ao buscar embalagens' }, { status: 500 })
  }
}
...
export async function POST(req: NextRequest) {
  try {
    ...
    await prisma.$executeRaw`
      ...
      VALUES (${id}, 'ws_atelier', ${nome}, ${descricao || null}, ${custoTotal}, true, NOW(), NOW())
    `
    ...
    for (const item of itens) {
      ...
      await prisma.$executeRaw`
        ...
        VALUES (
          ${itemId}, ${id},
          ${item.materialId || null}, ${item.nomeMaterial},
          ${Number(item.qtdUsada)}, ${Number(item.custoUnit)}, ${rend}
        )
      `
    }
